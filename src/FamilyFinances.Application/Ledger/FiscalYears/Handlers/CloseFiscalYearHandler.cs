...
public sealed class CloseFiscalYearHandler
{
    ...
    public CloseFiscalYearHandler(IFiscalYearGovernanceRepository governance, ILedgerUnitOfWork uow)
    {
        _governance = governance;
        _uow = uow;
    }

    public async Task<FiscalYearStatusDto> HandleAsync(CloseFiscalYearRequest request, CancellationToken ct)
    {
        await _governance.CloseYearAsync(request.Year, request.ActorUserId, ct);

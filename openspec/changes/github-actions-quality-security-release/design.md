...
- [MUST] Create exactly four dedicated workflows:
  - `ci-quality.yml`
...
- [MUST] Publish test results and coverage files as artifacts in `ci-quality`.
...
2. GitHub triggers:
   ...
   - `dependency-review.yml`
   - `codeql.yml`
3. `ci-quality` restores, builds, runs tests with coverage collector, uploads TRX + cobertura artifacts.
...
2. TRX and `coverage.cobertura.xml` files are uploaded as artifacts.
3. PR reviewer opens Checks -> `ci-quality`.
4. Reviewer inspects summary/logs and optionally downloads artifacts for deep inspection.
...
.github/workflows/
  ...
  codeql.yml
  release-windows.yml
  ci.yml (legacy, removed or reduced to avoid overlap)
```
...
| Test artifact publication | `actions/upload-artifact@v4` | add TRX/coverage paths | No |
...
### Decision 1: Split mixed CI/CD into four workflows
- **Choice:** Separate quality, dependency review, code scanning, and release packaging flows.
...
### Decision 5: First-phase coverage visibility via native GitHub artifacts and summaries
- **Choice:** Upload TRX and Cobertura output in `ci-quality`; no external service in this change.
- **Rationale:** Zero external dependency, immediate value, faster rollout.
...
- [ ] Confirm `ci-quality` runs on PRs to `develop`.
...
- [ ] Confirm TRX files are uploaded as artifacts.
- [ ] Confirm coverage files are uploaded as artifacts.
- [ ] Confirm artifact retention is explicitly configured.
- [ ] Confirm workflow summary includes enough info for reviewers.

### Dependency Review Workflow
- [ ] Confirm dependency-review runs only on pull requests.
- [ ] Confirm dependency-review targets `main` and `develop`.
...
- [ ] Confirm CodeQL runs on PRs to `develop`.
...
- [ ] Confirm release workflow artifact retention remains low (space-aware).
...
- [ ] Confirm `main` branch protection requires `codeql`.
...
- [ ] Confirm PR to `develop` still shows checks as informational.
...
- [ ] Confirm docs mention coverage visibility path in GitHub checks/artifacts.

...
#### Scenario: Table and chart export labels are explicit
- **WHEN** a report table exposes its existing export control
- **THEN** its visible label MUST state `CSV`
- **WHEN** a reusable reporting chart exposes its existing export control
- **THEN** its visible label MUST state `PNG`

...
- ❌ "I don't know the EXACT location of this UI element"
- ❌ "I'm not sure if we're reusing or creating this component"
- ❌ "The navigation flow has gaps"
- ❌ "I don't know the exact data model changes"
- ❌ "UX pattern isn't clearly defined"

**STOP and ASK the user immediately.** Don't proceed to artifact creation with gaps.

### Before Exiting Explore Mode

When user is ready to create artifacts (opsx:new or opsx:ff), mentally verify:
...
Mentally organize exploration notes like this structure (you'll use them for artifacts):

```markdown
NAVIGATION FLOWS:
- Flow 1: [name] - Start: [page], Steps: [...], End: [page]
- Flow 2: [name] - Start: [page], Steps: [...], End: [page]

COMPONENT REUSE:
- Reusing: [existing component] for [purpose] with [modifications]
- Creating New: [new component] because [reason]

UX PATTERNS:
- Interaction: [pattern description]
- Visual: [exact format/appearance]
- Inference: [logic/algorithm]

DATA MODEL:
- New Tables: [table names with columns]
- Extended Tables: [table name + new columns]
- Relationships: [FK descriptions]

SERVICES:
- New: [service name] with methods: [signatures]
- Extended: [service name] + new methods: [signatures]

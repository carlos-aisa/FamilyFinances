...
public sealed class ScriptLanHostOperationsServiceTests
{
    ...
    [Fact]
    public async Task ApplyAsync_ReturnsSucceededResult_WhenScriptsReturnJson()
    {
        if (!OperatingSystem.IsWindows())
            return;
    ...
    [Fact]
    public async Task ApplyAsync_ReturnsGenericFailureMessage_WhenScriptFailsWithoutAdminHint()
    {
        if (!OperatingSystem.IsWindows())
            return;
        ...
        try
        {
            ...
            result.Succeeded.Should().BeFalse();
            result.Message.Should().Be("LAN access operation failed. Check server logs for details.");
        }
        finally
        {
            TryDeleteDirectory(scriptsRoot);
    ...
    {
        var root = Path.Combine(Path.GetTempPath(), $"ff-web-hostops-tests-{Guid.NewGuid():N}");
